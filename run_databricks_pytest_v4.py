"""
Submit insurance-quantile pytest suite (v0.4.0, includes WassersteinRobustQR)
to Databricks serverless compute.

Runs both core tests (including test_robust.py) and EQRN tests.

Run from the project root:
    python run_databricks_pytest_v4.py
"""
...
WORKSPACE_FOLDER = "/Workspace/insurance-quantile-v040"
NOTEBOOK_PATH = f"{WORKSPACE_FOLDER}/run_pytest_v040"
...
# Smoke test: verify new module is present
print("Smoke test: _robust.py present ->", os.path.exists(f"{src_dir}/_robust.py"))
print("Smoke test: test_robust.py present ->", os.path.exists(f"{tests_dir}/test_robust.py"))
...
if r.returncode != 0:
    ...
    print("insurance-quantile v0.4.0 installed from", pkg_dir)

# COMMAND ----------

# Run core tests (includes test_robust.py)
...
run_resp = api_call("POST", "api/2.1/jobs/runs/submit", {
    "run_name": f"insurance-quantile-v040-pytest-{RUN_ID}",
    "tasks": [{
        "task_key": "pytest",
        "notebook_task": {
