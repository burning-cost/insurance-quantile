"""
Submit insurance-quantile pytest suite (v0.5.0, adds ExpectedShortfallRegressor)
to Databricks serverless compute.

Runs both core tests (including test_es_regressor.py) and EQRN tests.

Run from the project root:
    python run_databricks_pytest_v5.py
"""
...
WORKSPACE_FOLDER = "/Workspace/insurance-quantile-v050"
NOTEBOOK_PATH = f"{WORKSPACE_FOLDER}/run_pytest_v050"
...
# Smoke test: verify new modules are present
print("Smoke test: _es_regressor.py present ->", os.path.exists(f"{src_dir}/_es_regressor.py"))
print("Smoke test: test_es_regressor.py present ->", os.path.exists(f"{tests_dir}/test_es_regressor.py"))
...
if r.returncode != 0:
    ...
    print("insurance-quantile v0.5.0 installed from", pkg_dir)

# COMMAND ----------

# Run core tests (includes test_es_regressor.py)
...
run_resp = api_call("POST", "api/2.1/jobs/runs/submit", {
    "run_name": f"insurance-quantile-v050-pytest-{RUN_ID}",
    "tasks": [{
        "task_key": "pytest",
        "notebook_task": {
...
# Poll for completion
# ---------------------------------------------------------------------------
print("Polling for completion (timeout 25 min) ...")
