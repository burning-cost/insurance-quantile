"""
Submit insurance-quantile pytest suite (with expanded coverage batch 1)
to Databricks serverless compute.

Adds test_expanded_coverage_1.py with 109 new tests covering:
    - _es_regressor internals: _check_loss_scalar, _auto_bin_count edge cases,
      _assign_bins edge cases, _make_bin_edges degenerate, _local_es_estimate
      boundary, _pseudo_values properties
    - _model.py: _apply_isotonic, _to_numpy, _series_to_numpy helpers,
      QuantileGBM predict_tvar warning paths, predict_tvar series dtype,
      calibration_report mean_pinball_positive
    - _calibration.py: pinball_loss arithmetic, quantile_calibration_plot
      ImportError, coverage_check all-above/below, returns correct n rows
    - _types.py: QuantileSpec frozen, TVaRResult custom method, ExceedanceCurve
      column dtypes and single threshold, TwoPartResult field types
    - _robust.py: _check_loss, _c_tau_p for p=1/3, _intercept_correction p=1
      and eps=0, WassersteinRobustQR 1D predict raise, n_features_in_
    - __init__.py: lazy EQRN error, __version__, all exports accessible
    - _tvar.py: per_risk_tvar method label, var_values Series type
    - _two_part.py: _interpolate_severity_quantile all-invalid, exact quantile
      level, flat extrapolation, mixed valid/invalid
    - exceedance_curve: single risk, custom n_thresholds, oep_curve thresholds
    - _loading.py: column dtype, Polars Series mean model, ilf validation,
      MeanModelWrapper output
    - portfolio_tvar: sum vs mean arithmetic, error message, return type

Run from project root:
    python run_databricks_pytest_v7.py
"""
...
WORKSPACE_FOLDER = "/Workspace/insurance-quantile-v070"
NOTEBOOK_PATH = f"{WORKSPACE_FOLDER}/run_pytest_v070"
...
src_files = {
    ...
    "_robust.py":         read_file(f"{BASE}/src/insurance_quantile/_robust.py"),
    "_es_regressor.py":   read_file(f"{BASE}/src/insurance_quantile/_es_regressor.py"),
}
...
test_files = {
    ...
    "test_robust.py":                   read_file(f"{BASE}/tests/test_robust.py"),
    "test_es_regressor.py":             read_file(f"{BASE}/tests/test_es_regressor.py"),
    "test_new_coverage.py":             read_file(f"{BASE}/tests/test_new_coverage.py"),
    "test_expanded_coverage_1.py":      read_file(f"{BASE}/tests/test_expanded_coverage_1.py"),
}
...
# Run core tests (includes test_new_coverage.py and test_expanded_coverage_1.py)
...
print(r_core.stdout[-20000:])
if r_core.stderr:
    print("STDERR:", r_core.stderr[-2000:])
...
run_resp = api_call("POST", "api/2.1/jobs/runs/submit", {
    "run_name": f"insurance-quantile-v070-pytest-{RUN_ID}",
    "tasks": [{
        "task_key": "pytest",
        "notebook_task": {
